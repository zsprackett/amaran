...
enum Provisioning {
    static func ensureStatePath(_ statePath: String, add: Bool, context: String) throws {
        ...
        if add, !exists {
            throw CLIError("existing state is required at \(statePath) before adding a fixture; "
                + "run pair once without --add to create studio state")
        }
        if !add, exists {
            throw CLIError("state file already exists at \(statePath); "
                + "pass --add to append a fixture or set --state-path before \(context)")
        }
    }
    ...
    static func provisionSummary(_ data: JSONValue) -> String {
        let provisioning = data["native_provisioning"]
        var parts = ["provisioning complete: \(provisioning?["state_mode"]?.stringValue ?? "create") "
            + "address \(provisioning?["node_address"]?.intValue.map(String.init) ?? "?")"]
        if let path = provisioning?["state_path"]?.stringValue { parts.append("state \(path)") }
        if let suffix = provisioning?["device_uuid_suffix"]?.stringValue { parts.append("uuid ...\(suffix)") }
    ...
    static func configureOnce(_ opts: ResolvedOptions) throws -> JSONValue {
        let composition = try DiagnosticSupport.data(
            opts, flags: ["--config-composition-get-test"], kind: "config composition get")
        let appkeyList = try DiagnosticSupport.data(
            opts, flags: ["--config-appkey-get-test"], kind: "config appkey get")
        let appkeyAdd: JSONValue = appkeyListHasIndex(appkeyList, 0)
            ? .object([
                "skipped": .bool(true), "reason": .string("app_key_index_present"), "app_key_index": .int(0)
            ])
            : try DiagnosticSupport.data(opts, flags: ["--config-appkey-add-test"], kind: "config appkey add")
        let modelBind = try DiagnosticSupport.data(
    ...
    static func configureWithRetry(_ opts: ResolvedOptions) throws -> JSONValue {
        ...
        while true {
            do {
                ...
                FileHandle.standardError.write(Data(("amaran: configuration attempt \(attempt) failed; "
                    + "retrying in \(String(format: "%g", delay))s...\n").utf8))
...
public struct PairCommand: ParsableCommand {
    ...
    public func run() throws {
        let opts = options.resolve()
        if dryRun {
            let scan = try DiagnosticSupport.data(
            ...
            if opts.json {
                ...
                print("preflight: bluetooth \(scan["central_state"]?.stringValue ?? "unknown"), "
                    + "unprovisioned candidates \(found.count)")
            }
            return
        }
    ...
    public func run() throws {
        ...
        if dryRun {
            ...
            if opts.json {
                ...
                print("would reset \(Capabilities.label(fixture)) at address \(fixture.nodeAddress) "
                    + "(pass --confirm-reset)")
            }
            return
        }
        ...
        let data = try DiagnosticSupport.data(
            opts, flags: ["--config-node-reset-test", "--confirm-reset"], kind: "config node reset")
        if opts.json {
            print(JSONOutput.pretty(data))
        } else {
            print(DiagnosticSupport.sendSummary(data, "config node reset"))
        }
    }
}
