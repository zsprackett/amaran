...
enum DiagnosticSupport {
    ...
    static func sendSummary(_ data: JSONValue, _ kind: String) -> String {
        var parts: [String] = []
        if let address = data["native_send"]?["address"]?.intValue { parts.append("address \(address)") }
        if let sequence = data["native_send"]?["sequence"]?.intValue { parts.append("sequence \(sequence)") }
        if let count = data["proxy_write"]?["notification_count"]?.intValue { parts.append("notifications \(count)") }
...
public struct GattProbeCommand: ParsableCommand {
    ...
    public func run() throws {
        ...
        let data = try DiagnosticSupport.data(opts, flags: flags, includeState: false, includeNode: false, kind: "BLE probe")
        DiagnosticSupport.emit(data, json: opts.json) {
            let discoveries = data["discoveries"]?.arrayValue ?? []
            var lines = ["bluetooth \(data["central_state"]?.stringValue ?? "unknown"), mesh devices \(discoveries.count)"]
            for item in discoveries {
                let roles = (item["mesh_roles"]?.arrayValue?.compactMap { $0.stringValue } ?? ["unknown"]).joined(separator: ",")
...
public struct ProvisionScanCommand: ParsableCommand {
    ...
    public func run() throws {
        let opts = options.resolve()
        let data = try DiagnosticSupport.data(opts, flags: ["--provisioning-scan"], includeState: false, includeNode: false, kind: "provision scan")
        DiagnosticSupport.emit(data, json: opts.json) {
            let found = data["unprovisioned"]?.arrayValue ?? data["discoveries"]?.arrayValue ?? []
            return "provision scan: bluetooth \(data["central_state"]?.stringValue ?? "unknown"), candidates \(found.count)"
        }
    }
}
...
public struct ProvisionInviteTestCommand: ParsableCommand {
    ...
    public func run() throws {
        ...
        let data = try DiagnosticSupport.data(opts, flags: ["--provisioning-invite-test", String(duration)], includeState: false, includeNode: false, kind: "provision invite")
...
public struct ProxyTestCommand: ParsableCommand {
    ...
    public func run() throws {
        let opts = options.resolve()
        let data = try DiagnosticSupport.data(opts, flags: ["--proxy-test"], includeState: false, includeNode: false, kind: "proxy test")
...
public struct SigOnOffTestCommand: ParsableCommand {
    ...
    public func run() throws {
        let opts = options.resolve()
        guard value == "on" || value == "off" else { throw CLIError("sig-onoff-test requires on or off") }
...
public struct ControlTestCommand: ParsableCommand {
    ...
    @Option(name: .customLong("gm")) var gm: String?
    public init() {}
    public func run() throws {
        let opts = options.resolve()
        try ControlSupport.requireState(opts.statePath, kind: "control")
    ...
    private func buildSpec() throws -> String {
        ...
        case "cct":
            ...
            return "cct:\(value):\(intensity):\(gm ?? "10"):0"
        case "raw":
...
public struct ConfigCompositionGetTestCommand: ParsableCommand {
    ...
    public func run() throws {
        let opts = options.resolve()
        try ControlSupport.requireState(opts.statePath, kind: "control")
        let data = try DiagnosticSupport.data(opts, flags: ["--config-composition-get-test"], kind: "config composition get")
...
public struct ConfigModelAppBindTestCommand: ParsableCommand {
    ...
    public func run() throws {
        let opts = options.resolve()
        try ControlSupport.requireState(opts.statePath, kind: "control")
        let data = try DiagnosticSupport.data(opts, flags: ["--config-model-app-bind-test"], kind: "config model app bind")
