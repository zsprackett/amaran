...
struct DaemonReportTests {
    ...
    @Test func humanStatusFormatsRunningDaemon() throws {
        ...
        #expect(DaemonReport.humanStatus(report) == "daemon running: pid 86320, bluetooth poweredOn, connected no")
    }

    @Test func humanStatusFormatsConnected() throws {
        let report = try response(
            "{\"ok\":true,\"data\":{\"daemon\":{\"pid\":1},\"central_state\":\"poweredOn\",\"connected\":true}}")
        #expect(DaemonReport.humanStatus(report) == "daemon running: pid 1, bluetooth poweredOn, connected yes")
    }

    @Test func humanStatusReportsError() throws {
        let report = try response("{\"ok\":false,\"error\":\"Bluetooth is poweredOff\"}")
        #expect(DaemonReport.humanStatus(report) == "daemon error: Bluetooth is poweredOff")
    }
}
