...
struct FixtureCommandsTests {
    ...
    @Test func listEntriesAreRedactedSummaries() {
        ...
        let entry = entries[0]
        #expect(entry.name == "key")            // friendly wins
        #expect(entry.friendlyName == "key")
        #expect(entry.sourceName == "400M5-DDEEFF")
        #expect(entry.address == 6)
        #expect(entry.mac == "AA:BB:CC:DD:EE:FF")
        #expect(entry.capabilities.cctMin == 2700)   // 400M5 family
        #expect(entry.capabilities.gmSupported == false)
    }
    ...
    @Test func renameSetsFriendlyNameAndPreservesOtherFields() throws {
        var currentState = state([fixture(6, name: "alpha", code: "X", mac: "AABBCCDDEEFF")])
        let result = try FixtureOps.rename(state: &currentState, selector: "6", name: "key")
        #expect(result.renamed == true)
        #expect(result.previousFriendlyName == nil)
        #expect(currentState.fixtures[0].friendlyName == "key")
        #expect(currentState.fixtures[0].extra["updated_at"] != nil)   // timestamp stamped
        #expect(currentState.fixtures[0].code == "X")                  // untouched
    }

    @Test func renameRejectsInvalidNames() {
        var currentState = state([fixture(6, name: "alpha")])
        #expect(throws: CLIError.self) { try FixtureOps.rename(state: &currentState, selector: "6", name: "") }
        #expect(throws: CLIError.self) { try FixtureOps.rename(state: &currentState, selector: "6", name: "has space") }
        // numeric
        #expect(throws: CLIError.self) {
            try FixtureOps.rename(state: &currentState, selector: "6", name: "6")
        }
        // hex numeric
        #expect(throws: CLIError.self) {
            try FixtureOps.rename(state: &currentState, selector: "6", name: "0x10")
        }
    }

    @Test func renameRejectsConflictWithOtherSelector() {
        var currentState = state([fixture(6, name: "alpha"), fixture(7, name: "beta")])
        #expect(throws: CLIError.self) { try FixtureOps.rename(state: &currentState, selector: "6", name: "beta") }
    }

    @Test func clearNameRemovesFriendlyName() throws {
        var currentState = state([fixture(6, name: "alpha", friendly: "key")])
        let result = try FixtureOps.clearName(state: &currentState, selector: "6")
        #expect(result.cleared == true)
        #expect(result.previousFriendlyName == "key")
        #expect(currentState.fixtures[0].friendlyName == nil)
    }
