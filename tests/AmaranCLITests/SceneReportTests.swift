...
struct SceneReportTests {
    ...
    private let sceneA = Scene(capturedAt: "2026-01-01T00:00:00Z", fixtures: [
        SceneFixture(nodeAddress: 3, name: "key", macSuffix: "AABBCC",
                     intensity: 75.0, cct: 5600, greenMagenta: 4, sleepMode: 1)
    ])
    ...
    @Test func showReturnsSummaryOrThrows() throws {
        let sceneState = state(scenes: ["warm": sceneA])
        let summary = try SceneReport.show(sceneState, name: "warm")
        #expect(summary.fixtures.first?.cct == 5600)
        #expect(throws: CLIError.self) { _ = try SceneReport.show(sceneState, name: "missing") }
    }

    @Test func deleteRemovesSceneAndFlags() throws {
        var sceneState = state(scenes: ["warm": sceneA, "cool": sceneB])
        let result = try SceneReport.delete(state: &sceneState, name: "warm")
        #expect(result.deleted == true)
        #expect(sceneState.scenes?["warm"] == nil)
        #expect(sceneState.scenes?["cool"] != nil)
        #expect(throws: CLIError.self) { _ = try SceneReport.delete(state: &sceneState, name: "warm") }
    }

    @Test func showLinesRenderFixtureDetail() throws {
        let sceneState = state(scenes: ["warm": sceneA])
        let summary = try SceneReport.show(sceneState, name: "warm")
        let lines = SceneReport.showLines(summary, state: sceneState)
